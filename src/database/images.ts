...
export async function getImageForFolder(
    folderId: number
...
export function deleteImage(imageId: number) {
    return supabase.from("gallery_images").delete().eq("id", imageId);
}

export function deleteImagesByFolderId(folderId: string | number) {
    return supabase.from("gallery_images").delete().eq("folder_id", folderId);
}

export async function updateImageFolder(
