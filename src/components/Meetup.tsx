...
const Meetup: React.FC = () => {
    ...
    return (
        <div className="page">
            <h1 className="about-header" style={{ marginBottom: 10 }}>Club Meetups & Events</h1>
            <p style={{ marginBottom: 50 }}>
                Join us for regular meetings, operating sessions, and training events. Whether you're a seasoned operator or just starting out, there's something for everyone.
            </p>
            <section className="meetups-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 24, marginBottom: 48 }}>
                {meetups.map((meetup) => (
                    <div key={meetup.id} className="meetup-card" style={{ background: "var(--panel)", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                        <div style={{ height: 200, overflow: "hidden", background: "rgba(0,0,0,0.05)" }}>
                            <img src={meetup.image} alt={meetup.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                        </div>

                        <div style={{ padding: 20 }}>
                            <h3 style={{ marginTop: 0, marginBottom: 8, fontSize: 18, fontWeight: 700 }}>{meetup.title}</h3>

                            <div style={{ display: "grid", gap: 6, marginBottom: 12, fontSize: 14, color: "var(--muted, #666)" }}>
                                <div>
                            ...
                            <p style={{ margin: "12px 0", lineHeight: 1.5, color: "var(--text, #333)", minHeight: 50 }}>
                                {meetup.description}
                            </p>
                        </div>
                    </div>
            ...
            <section style={{ maxWidth: 800, marginLeft: "auto", marginRight: "auto", padding: 24, background: "var(--panel)", borderRadius: 12 }}>
                <h2 style={{ marginTop: 0 }}>Getting Started</h2>
                <p>
                    New to amateur radio or our club? All meetups are open to licensed amateurs and those interested in learning.
                    No experience necessary — we welcome beginners!
                </p>
                ...
                <div style={{ marginTop: 20, padding: 16, background: "rgba(168,59,42,0.08)", borderLeft: "4px solid var(--accent)", borderRadius: 6 }}>
                    <strong>📢 Questions?</strong> Contact us at <a href="mailto:es1mw@erau.ee">es1mw@erau.ee</a> or find us on the air.
                </div>
            </section>
        </div>
    );
